  ...
  useEffect(() => {
    const fetchTechnicianSuggestions = async () => {
      try {
        ...
        const serviceCategories = selectedServices.map(s => s.category || s.name);
        ...
        if (techniciansResponse.data && shopsResponse.data) {
          const enhancedTechnicians = techniciansResponse.data.map(tech => ({
            ...tech,
            shop: shopsResponse.data.find(shop => shop.id === tech.shop_id)
          }));
      ...
      <CardContent className="space-y-4">
        {suggestions.length === 0 ? (
          <div className="text-center py-6 text-muted-foreground">
            <User className="h-12 w-12 mx-auto mb-4 opacity-50" />
            <p>No technicians found with matching specializations</p>
          </div>
        ) : (
            ...
            >
              ...
              <div className="flex items-center justify-between pt-2 border-t">
                <div className="flex items-center gap-2 text-sm text-muted-foreground">
                  ...
                  <span>{suggestion.technician.phone}</span>
                </div>
                <Button 
                  size="sm"
